...
#include "GameModes/LaryLearnGameMode.h"
...
ALaryLearnGameMode::ALaryLearnGameMode(const FObjectInitializer& ObjectInitializer)
...
const ULyraPawnData* ALaryLearnGameMode::GetPawnDataForController(const AController* InController) const
{
...
void ALaryLearnGameMode::InitGame(const FString& MapName, const FString& Options, FString& ErrorMessage)
{
...
void ALaryLearnGameMode::HandleMatchAssignmentIfNotExpectingOne()
{
...
void ALaryLearnGameMode::OnMatchAssignmentGiven(FPrimaryAssetId ExperienceId, const FString& ExperienceIdSource)
{
#if WITH_SERVER_CODE
	if (ExperienceId.IsValid())
	{
...
void ALaryLearnGameMode::OnExperienceLoaded(const ULyraExperienceDefinition* CurrentExperience)
{
...
bool ALaryLearnGameMode::IsExperienceLoaded() const
{
	check(GameState);
	ULyraExperienceManagerComponent* ExperienceComponent = GameState->FindComponentByClass<ULyraExperienceManagerComponent>();
	check(ExperienceComponent);
...
UClass* ALaryLearnGameMode::GetDefaultPawnClassForController_Implementation(AController* InController)
{
...
APawn* ALaryLearnGameMode::SpawnDefaultPawnAtTransform_Implementation(AController* NewPlayer, const FTransform& SpawnTransform)
{
...
bool ALaryLearnGameMode::ShouldSpawnAtStartSpot(AController* Player)
{
	// We never want to use the start spot, always use the spawn management component.
	return Super::ShouldSpawnAtStartSpot(Player);
}

void ALaryLearnGameMode::HandleStartingNewPlayer_Implementation(APlayerController* NewPlayer)
{
...
AActor* ALaryLearnGameMode::ChoosePlayerStart_Implementation(AController* Player)
{
...
void ALaryLearnGameMode::FinishRestartPlayer(AController* NewPlayer, const FRotator& StartRotation)
{
	//TODO
	// if (ULyraPlayerSpawningManagerComponent* PlayerSpawningComponent = GameState->FindComponentByClass<ULyraPlayerSpawningManagerComponent>())
	// {
...
bool ALaryLearnGameMode::PlayerCanRestart_Implementation(APlayerController* Player)
{
	return ControllerCanRestart(Player);
}

bool ALaryLearnGameMode::ControllerCanRestart(AController* Controller)
{
...
void ALaryLearnGameMode::InitGameState()
{
	Super::InitGameState();
...
void ALaryLearnGameMode::OnPostLogin(AController* NewPlayer)
{
...
void ALaryLearnGameMode::RequestPlayerRestartNextFrame(AController* Controller, bool bForceReset)
{
...
bool ALaryLearnGameMode::UpdatePlayerStartSpot(AController* Player, const FString& Portal, FString& OutErrorMessage)
{
...
void ALaryLearnGameMode::FailedToRestartPlayer(AController* NewPlayer)
{
