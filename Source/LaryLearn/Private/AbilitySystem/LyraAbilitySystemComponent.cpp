...
#include "AbilitySystem/Abilities/LyraAbilityTagRelationshipMapping.h"
	...
	for (const FGameplayAbilitySpec& AbilitySpec : ActivatableAbilities.Items)
	{
		...
		if (LyraAbilityCDO->GetInstancingPolicy() != EGameplayAbilityInstancingPolicy::NonInstanced)
		{
			...
			for (UGameplayAbility* AbilityInstance : Instances)
			{
				...
				if (ShouldCancelFunc(LyraAbilityInstance, AbilitySpec.Handle))
				{
					...
					{
						LyraAbilityInstance->CancelAbility(AbilitySpec.Handle, AbilityActorInfo.Get(),
		...
		else
		{
			...
			if (ShouldCancelFunc(LyraAbilityCDO, AbilitySpec.Handle))
			{
				...
				LyraAbilityCDO->CancelAbility(AbilitySpec.Handle, AbilityActorInfo.Get(),
	...
	for (const FGameplayAbilitySpecHandle& SpecHandle : InputPressedSpecHandles)
	{
 		if (FGameplayAbilitySpec* AbilitySpec = FindAbilitySpecFromHandle(SpecHandle))
		{
			if (AbilitySpec->Ability)
			{
				...
				{
					const ULyraGameplayAbility* LyraAbilityCDO = CastChecked<
						ULyraGameplayAbility>(AbilitySpec->Ability);
...
void ULyraAbilitySystemComponent::HandleAbilityFailed(const UGameplayAbility* Ability,
                                                      const FGameplayTagContainer& FailureReason)
{
...
{
	const TSubclassOf<UGameplayEffect> DynamicTagGE = ULyraAssetManager::GetSubclass(
		ULyraGameData::Get().DynamicTagGameplayEffect);
	if (!DynamicTagGE)
	{
		UE_LOG(LogTemp, Warning, TEXT("AddDynamicTagGameplayEffect: Unable to find DynamicTagGameplayEffect [%s]."),
	...
	if (!Spec)
	{
		UE_LOG(LogTemp, Warning, TEXT("AddDynamicTagGameplayEffect: Unable to make outgoing spec for [%s]."),
...
void ULyraAbilitySystemComponent::RemoveDynamicTagGameplayEffect(const FGameplayTag& Tag)
{
	...
	if (!DynamicTagGE)
	{
		UE_LOG(LogTemp, Warning, TEXT("RemoveDynamicTagGameplayEffect: Unable to find gameplay effect [%s]."),
		       *ULyraGameData::Get().DynamicTagGameplayEffect.GetAssetName());
		return;
	}
